...
type HomeData = {
  ...
  comics: { id: number; title: string; coverPath: string | null; category: string; _count: { pages: number } }[];
  posts: { id: number; slug: string; title: string; createdAt: string }[];
  games: { id: number; slug: string; title: string; description: string; icon: string; builtin: boolean }[];
  poems: { id: number; title: string; author: string; dynasty: string; content: string }[];
};
...
function Spine({ title, id }: { title: string; id: number }) {
  ...
  return (
    ...
    >
      <span aria-hidden className="absolute inset-x-1 top-0 h-px bg-paper/25" />
  ...
  if (failed) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-3 px-4 text-center">
        <p className="font-serif text-xl">书房的灯暂时没点上</p>
        <p className="text-sm text-fog">连不上服务器，请稍后再试。</p>
      </div>
    );
  }
      ...
      <div>
        <section className="lamp-glow border-b hairline">
          <div className="mx-auto max-w-5xl px-4 pb-16 pt-20 sm:px-6 sm:pb-20 sm:pt-24">
            <div className="mb-6 h-3 w-24 animate-pulse rounded bg-ink-2" />
            <div className="h-12 w-72 max-w-full animate-pulse rounded bg-ink-2" />
            <div className="mt-8 flex gap-3">
              <div className="h-10 w-32 animate-pulse rounded-md bg-ink-2" />
              <div className="h-10 w-28 animate-pulse rounded-md bg-ink-2" />
            </div>
          </div>
        </section>
        <div className="mx-auto max-w-5xl space-y-10 px-4 py-14 sm:px-6">
          <div className="h-24 animate-pulse rounded-lg bg-ink-2/60" />
          <div className="h-40 animate-pulse rounded-lg bg-ink-2/60" />
        </div>
      </div>
    );
  }

  const { counts, books, comics, posts, games, poems } = data;
  const stats = [
    { n: counts.books, label: "本藏书" },
    { n: counts.comics, label: "部漫画" },
    { n: counts.poems, label: "首诗词" },
    { n: counts.posts, label: "篇随笔" },
    { n: counts.games, label: "个游戏" },
  ];
